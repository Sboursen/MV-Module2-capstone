  ...
  carApi.getDataPromise().then((data) => {
    ...
    <form class='form  shadow-md rounded px-8 pt-6 pb-8 mb-4'>
         <h2 class='book-cars-title'> Add a reservation </h2>
          ...
          <button class='reserve-car-btn center bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mb-6'> Reserve </button>
        </form>
    </div>
    </div>   
    `;
    ...
    const reserveCarBtn = document.querySelector('.reserve-car-btn');
    reserveCarBtn.addEventListener('click', (e) => {
      e.preventDefault();
      form.reset();
      setReservedToAPI(id);
    });
